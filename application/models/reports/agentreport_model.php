...
class agentreport_Model extends CI_Model
{
	...
	public function getPartnersTurnover($data){
		...
		if($partner_type==0 || $partner_type==''){
			...
			if(!empty($data["PARTNER_TYPE"]) ) {//partnerid
				if($data["PARTNER_TYPE"] == 15){
					$this->db3->select('SUPERDISTRIBUTOR_ID,SUPERDISTRIBUTOR_NAME,sum(PLAY_POINTS) as totbet, sum(WIN_POINTS) as totwin,sum(MARGIN) as MARGIN,sum(NET) as NET,MARGIN_PERCENTAGE')
				...
				}elseif($data["PARTNER_TYPE"] == 12){
					$this->db3->select('DISTRIBUTOR_ID,DISTRIBUTOR_NAME,sum(PLAY_POINTS) as totbet, sum(WIN_POINTS) as totwin,sum(MARGIN) as MARGIN,sum(NET) as NET,MARGIN_PERCENTAGE')
				...
				}elseif($data["PARTNER_TYPE"] == 13){
					$this->db3->select('SUBDISTRIBUTOR_ID,SUBDISTRIBUTOR_NAME,sum(PLAY_POINTS) as totbet, sum(WIN_POINTS) as totwin,sum(MARGIN) as MARGIN,sum(NET) as NET,MARGIN_PERCENTAGE')
				...
				}elseif($data["PARTNER_TYPE"] == 14){
					$this->db3->select('PARTNER_ID,PARTNER_NAME,sum(PLAY_POINTS) as totbet, sum(WIN_POINTS) as totwin,sum(MARGIN) as MARGIN,sum(NET) as NET,MARGIN_PERCENTAGE')
			...
			if($data["GAMES_TYPE"] != 'all' ) {
				//$this->db3->where("GAMES_ID",$data["GAMES_TYPE"]); 
			}
		...
		if($partner_type==11){
			...
			/* if(!empty($data["PARTNER_TYPE"]) ) {//partnerid
				$sql = "select partner_id from partner where fk_partner_id=".$data["partnerid"]." and fk_partner_type_id=".$data["PARTNER_TYPE"];
				if($data["PARTNER_TYPE"] == 15){
					$this->db3->where("SUPERDISTRIBUTOR_ID IN( $sql )");
				}elseif($data["PARTNER_TYPE"] == 12){
					$this->db3->where("DISTRIBUTOR_ID IN( $sql )");
				}elseif($data["PARTNER_TYPE"] == 13){
					$this->db3->where("SUBDISTRIBUTOR_ID IN( $sql )");
				}elseif($data["PARTNER_TYPE"] == 14){
					$this->db3->where("PARTNER_ID IN( $sql )");
				}
		...
		if($partner_type==12){
		   ...
		   #Sub dist results
			if(!empty($data["PARTNER_TYPE"]) && $data["PARTNER_TYPE"] == 14 ) {//partnerid 
				$this->db3->select('PARTNER_ID,PARTNER_NAME,sum(PLAY_POINTS) as totbet, sum(WIN_POINTS) as totwin,sum(MARGIN) as MARGIN,sum(NET) as NET,MARGIN_PERCENTAGE')
					->from('yagent_game_turn_over_history');
					if($data["GAMES_TYPE"] != 'all' ) {
						if($data["GAMES_TYPE"] == 'pc' ) {
		...
		*/
		if($partner_type==0){
			...
			if(!empty($data['GAMEID'])){
				$this->db3->where('GAME_ID',$data['GAMEID']);
			}
	...
	public function getPartnersTurnoverGameWise($data){
		...
		if($partner_type==0){
			...
			$this->db3->where("MAIN_AGEN_ID",$partid);
    			if($_REQUEST["GAMES_TYPE"] != 'all' ) {
     				if($_REQUEST["GAMES_TYPE"] == 'pc' ) {
					$this->db3->where_not_in("GAME_ID",unserialize(MOBILE_GAMES));
...
// 					echo $this->db3->last_query(); die;
	...
	public function getSubDistributorTurnover($data){
		...
		if($partner_type==15 ||  $partner_type==11 || $partner_type==12 || $partner_type==13){
			...
			if($data['PARTNER_TYPE']==13){
				$this->db3->where("MAIN_AGEN_ID",$distributor_id);
			}else{
				$this->db3->where("SUBDISTRIBUTOR_ID",$distributor_id);
			}
			if($_REQUEST["GAMES_TYPE"] != 'all' ) {
	...
	public function getAgentTurnoverGameWise($data){
		...
		if($partner_type==15 || $partner_type==11 || $partner_type==12 || $partner_type==13 || $partner_type==0 || $partner_type==14){
			$this->db3->select('PARTNER_ID,PARTNER_NAME,sum(PLAY_POINTS) as totbet, sum(WIN_POINTS) as totwin,sum(MARGIN) as MARGIN,sum(NET) as NET,MARGIN_PERCENTAGE,GAME_ID,PARTNER_COMMISSION_TYPE,GAME_DESCRIPTION')->from('yagent_game_turn_over_history');
...
// 		echo $this->db3->last_query(); exit;
   ...
   //echo $this->db3->last_query(); die;
	...
	public function getPartnerList($partnerid,$partner_type) {
		$arrPartnerIDs=explode(",",$partnerid);
		$this->db2->select("PARTNER_ID")->from("partner");
		$this->db2->where_in("FK_PARTNER_ID",$arrPartnerIDs);
