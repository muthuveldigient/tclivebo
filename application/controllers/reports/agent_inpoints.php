...
class Agent_inpoints extends CI_Controller{
    ...
    function __construct(){
	  parent::__construct();
			...
			$this->load->model('reports/agentinpoints_model');
	...
	public function inpoints() {
		$this->load->library('pagination');
		$config['base_url']  = base_url()."reports/agent_inpoints/inpoints/";
		...
		if($this->input->get_post('keyword',TRUE)=="Search") {		
			...
			$data['results']		= $this->agentinpoints_model->getInpointsInfo($loggedInPartnersList,$searchdata,$config["per_page"],$start);
		...
		$partners_list=$this->agentinpoints_model->getPartnersNameList($totPartnersList);
		$data['rid'] = $rid;
		$data['loggedInPartnersList'] = $partners_list;
		$this->load->view('reports/inpoints',$data);		
	}	
	
	public function inpointshistory(){
	  
		//general configuration
		$this->load->library('pagination');
		$config['base_url']  = base_url()."reports/agent_inpoints/inpointshistory/";
		$config['per_page']  = $this->config->item('limit');
		if(isset($paymentDateRange) && isset($dType))
			$start = $this->uri->segment(6,0);
		else
       ...
       // $searchdata['agent_list'] = 19;
		...
		//	$this->pagination->initialize($config);	
			...
			$data['results']		= $this->agentinpoints_model->getAllSearchLedger($loggedInPartnersList,$searchdata,$config["per_page"],$start);
		...
		$this->load->view('reports/agent_inpoints',$data);   
	
	}
	
	
	public function agent_inpointshistory(){
       ...
       // $searchdata['agent_list'] = 19;
		...
		if($this->input->get_post('keyword',TRUE)=="Search"){		
			...
			$data['results']		= $this->agentinpoints_model->getAllAgentSearchLedger($loggedInPartnersList,$searchdata,$config["per_page"],$start);
		...
		$this->load->view('reports/agent_inpoints_details',$data);   
	
	}
	
	
	public function user_inpointshistory(){
