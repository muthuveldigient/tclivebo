		...
		for($i=0;$i<count($results);$i++){
			  ...
			  if($results[$i]->USER_ID!="0"){  
					...
					if($results[$i]->INTERNAL_REFERENCE_NO){
						$userbalance=$this->agentoutpoints_model->getUserBalanceById($results[$i]->INTERNAL_REFERENCE_NO);
						...
						if($userbalance[0]->CURRENT_TOT_BALANCE){
							...
							if($CURRENT_TOT_BALANCE>$CLOSING_TOT_BALANCE){ $resultValue['AMOUNT_OUT'] = number_format($results[$i]->AMOUNT,2,".","");$resultValue1['AMOUNT_OUT'] = $results[$i]->AMOUNT;} else if($CURRENT_TOT_BALANCE==$CLOSING_TOT_BALANCE){ $resultValue['AMOUNT_OUT'] = '0';$resultValue1['AMOUNT_OUT'] = '0';} else{ $resultValue['AMOUNT_OUT'] = '';$resultValue1['AMOUNT_OUT'] = '';}
			  ...
			  if($results[$i]->SUB_PARTNER_ID!="0" && $results[$i]->SUB_PARTNER_ID!=''){
				...
				$partnername=$this->agentoutpoints_model->getPartnerId($results[$i]->PARTNER_ID);
				...
				$resultValue['USERNAME'] = $partnername[0]->PARTNER_USERNAME;
				  ...
				  if($session_data['partnertypeid']=="11"){
					...
					$resultValue['USERNAME'] = $partnerusername[0]->PARTNER_USERNAME;
					...
					$PROCESSED_BY=$partnertransbal[0]->PROCESSED_BY;
					 ...
					 if($results[$i]->CURRENT_TOT_BALANCE>$results[$i]->CLOSING_TOT_BALANCE){$resultValue['AMOUNT_OUT'] = number_format($results[$i]->AMOUNT,2,".","");$resultValue1['AMOUNT_OUT'] = $results[$i]->AMOUNT; } else if($results[$i]->CURRENT_TOT_BALANCE==$results[$i]->CLOSING_TOT_BALANCE){ $resultValue['AMOUNT_OUT']='0'; } else {$resultValue['AMOUNT_OUT'] = '';$resultValue1['AMOUNT_OUT'] = '';}
					 
					$resultValue['CURRENT_TOT_BALANCE'] = number_format($results[$i]->CURRENT_TOT_BALANCE,2,".","");
					$resultValue1['CURRENT_TOT_BALANCE'] = $results[$i]->CURRENT_TOT_BALANCE; 
					
					$resultValue['CLOSING_TOT_BALANCE'] = number_format($results[$i]->CLOSING_TOT_BALANCE,2,".","");
					$resultValue1['CLOSING_TOT_BALANCE'] = $results[$i]->CLOSING_TOT_BALANCE; 
			...
			if($resultValue['AMOUNT_OUT']!="" && $resultValue['AMOUNT_OUT']!="0.00" && !in_array($resultValue1['USERNAME']."-".$resultValue1['PARTNER_USERNAME'],$getValues) &&  $resultValue1['USERNAME']!=$resultValue['PARTNER_USERNAME'] ){   
    ...
    <div class="tableListWrap">
      <div class="data-list">
        ...
        <script type="text/javascript">
                jQuery("#list4").jqGrid({
                    datatype: "local",
						colNames:['Trans.Id','Date', 'User Id','Old Points','Out','New Points','Processed By', 'Comment'],
   						colModel:[
