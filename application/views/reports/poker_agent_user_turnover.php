  ...
  <div class="RightWrap">
    <div class="content_wrap">
      ...
      <?php if(isset($_REQUEST['errmsg'])){
		...
		if(isset($arrs) && $arrs!=''){
			if($partnertype==11 || $partnertype==15 || $partnertype==12 || $partnertype==13 || $partnertype==0){	 ?>
<style>
...
jQuery("#listsg11").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/poker_agent_userreport/<?php echo $agntid; ?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search',
	datatype: "xml",
	height: '100%',
	width: 1002,
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/poker_subgrid_gamewise_agentreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'',
			datatype: "xml",
			colNames: ['Game','Play Points','Win Points','Total Rake','Agent Rake','Margin%','Type'],
			colModel: [
				{name:"GAME_ID",index:"GAME_ID",width:180,key:true},
				{name:"PLAY_POINTS",index:"PLAY_POINTS",width:110,align:"right", sorttype: 'float'},
        ...
        {name:"TOTAL_RAKE",index:"TOTAL_RAKE",width:130,align:"right", sorttype: 'float'},
				{name:"MARGIN",index:"MARGIN",width:130,align:"right", sorttype: 'float'},
				//{name:"NET",index:"NET",width:130,align:"right", sorttype: 'float'},
            ...
            <script type="text/javascript">
                jQuery("#list6").jqGrid({
                    ...
                    colModel:[
						...
						{name:'TOTALRAKE',index:'TOTALRAKE', align:"right", width:40,summaryType: 'sum', sorttype: 'float', formatter: 'float'},
