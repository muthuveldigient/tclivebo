  ...
  <div class="RightWrap">
    <div class="content_wrap">
      ...
      <?php if(isset($_REQUEST['errmsg'])){
		...
		$magntresults=$dresults['magnt_result'];
		$distresults=$dresults['superdist_result'];
			...
			$agntvalue['NET2'] = $partner_comm;
              ...
              if(count($distresults)>0 && is_array($distresults)){
					for($i=0;$i<count($distresults);$i++){
						//get partner name
						$partnername	  = $distresults[$i]->SUPERDISTRIBUTOR_NAME;
...
jQuery("#listsg11").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/poker_magntreport/<?php echo $magntid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search',
	datatype: "xml",
	height: '100%',
	width: 1002,
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/poker_subgrid_gamewise_magntreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'',
			datatype: "xml",
...
jQuery("#listsg13").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/poker_magntsuperdistreport/<?php echo $magntid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search',
	datatype: "xml",
	height: '100%',
	width: 1002,
   	colNames:['Partner Name','Play Points', 'Win Points','Player Loss', 'Total Rake','Agent Rake','Company'],
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		$("#"+subgrid_id).html("<table id='"+subgrid_table_id+"' class='scroll'></table><div id='"+pager_id+"' class='scroll'></div>");
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/poker_subgrid_gamewise_magntsuperdistreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'&superdistid=<?php echo $distid;?>',
			datatype: "xml",
			colNames: ['Game','Play Points','Win Points','Total Rake','Agent Rake','Margin%','Type'],
			colModel: [
				{name:"GAME_ID",index:"GAME_ID",width:180,key:true},
