		...
		$agent_results 	 = $results['agent_results'];
			...
			if(count($agent_results)>0 && is_array($agent_results)){
                      for($l=0;$l<count($agent_results);$l++){
							//get partner name
							$partnername	  = '<a href="'.base_url().'reports/agent_turnover/agentpokerreport/'.$agent_results[$l]->PARTNER_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$agent_results[$l]->PARTNER_NAME.'</a>';
						//get partner revenueshare
						//$share	 = $this->Agent_model->getRevenueShareByPartnerId($results[$i]->partner_id);
...
jQuery("#listsg12").jqGrid({
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/poker_subgrid_gamewise_subdistreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&GAMES_TYPE='+"<?php echo $sGameType;?>"+'&agentid='+"<?php echo $sAgentID;?>"+'&keyword=Search',
			datatype: "xml",
