...
<div class="content_wrap">
	...
	echo form_open('reports/agent_inpoints/inpoints?rid='.$rid,$attributes);
...
if(isset($results)){ 
	foreach($results as $lIndex=>$lData) {
		if(!empty($lData->USER_ID)) {
			...
			$resultValue['USERNAME'] = "<a href='".base_url()."reports/agent_inpoints/agent_inpointshistory?rid=".$rid."&parid=".base64_encode($lData->PARTNER_ID)."&START_DATE_TIME=".date("d-m-Y H:i:s",strtotime($sdate))."&END_DATE_TIME=".date("d-m-Y H:i:s",strtotime($edate))."&keyword=Search'>".$lData->PARTNER_USER_NAME."</a>";
		}
...
if(isset($arr) && $arr!=''){ ?>
    <div class="tableListWrap">
      <div class="data-list">
       ...
       <div style="padding-top:10px;float:right;font-family:Trebuchet MS",Arial,Helvetica,sans-serif;font-size:11px;padding-right:10px;">Total Points: <strong><?php echo number_format($totalinpts,2,".","");?></strong></div>         
      </div>
    </div>
    <?php }else{ ?>
	<div class="tableListWrap">
