...
<div class="content_wrap">
      ...
      <?php if(isset($_REQUEST['errmsg'])){  echo $this->common_model->errorHandling($_REQUEST['errmsg']); } ?> 
   		  ...
   		  <form action="<?php echo base_url();?>reports/turnover/listgame?rid=<?php echo $rid;?>" method="post" name="tsearchform" id="tsearchform">
                                   ...
                                   if(isset($results)){ 
										if(count($results)>0 && is_array($results)){
											...
											for($i=0;$i<count($results);$i++){
													...
													$currencyType	= $this->report_model->getCurrencyNameByID($results[$i]->COIN_TYPE_ID);
													...
													if($dailytournament_ids){
														for($j=0;$j<count($dailytournament_ids);$j++){
														
															if($dtid)
															$dtid=$dtid.",".$dailytournament_ids[$j]->DAILY_TOURNAMENT_ID;
															else
															$dtid=$dailytournament_ids[$j]->DAILY_TOURNAMENT_ID;
														}
													
														if($dtid){
														$totalplayers= $this->report_model->getGamesTOTotalPlayers($dtid);	
														}else{
														$totalplayers="";
														}
													...
													if($results[$i]->totalpot_amounts)
													$resvalue['TOTAL_POT_AMOUNT']	= $results[$i]->totalpot_amounts;
													else
													$resvalue['TOTAL_POT_AMOUNT']	= '-';
													
													if($results[$i]->totalgames)
													$resvalue['TOTAL_GAMES']	= '<a href="'.base_url().'games/poker/game/view/?tableID='.$results[$i]->TOURNAMENT_NAME.'&keyword=Search&START_DATE_TIME='.$START_DATE_TIME.'&END_DATE_TIME='.$END_DATE_TIME.'&rid=51">'.$results[$i]->totalgames.'</a>';
													else
													...
													$resvalue['TOTAL_REVENUE']	= '<a href="'.base_url().'games/poker/game/view/?tableID='.$results[$i]->TOURNAMENT_NAME.'&keyword=Search&START_DATE_TIME='.$START_DATE_TIME.'&END_DATE_TIME='.$END_DATE_TIME.'&rid=51">'.$results[$i]->total_revenue.'</a>';
													else
    ...
    <div class="tableListWrap">
      <div class="data-list">
        ...
        <script type="text/javascript">
                jQuery("#list4").jqGrid({
                    datatype: "local",
					colNames:['Table ID','Stake','Games Played','Total Pot Amount','Total Player','Total Revenue','Currency','Status'],
                    colModel:[
						...
						{name:'TOTAL_POT_AMOUNT',index:'TOTAL_POT_AMOUNT', align:"center", width:70},
