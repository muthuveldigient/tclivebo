  ...
  <div class="RightWrap">
	...
	//	print_r($results);
	    ...
	    $resvalues=array();
			...
			//$presvalue['SNO1']=$k+1;
			$presvalue['PARTNER1']= '<a href="'.base_url().'reports/agent_turnover/agentpokerreport/'.$subdist_results[$l]->PARTNER_ID.'?rid=19&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'">'.$partnername.'</a>';
			...
			$presvalue['ENDPOINTS1'] = $partner_comm;
				...
				if(count($agent_results)>0 && is_array($agent_results)){
                      ...
                      for($l=0;$l<count($agent_results);$l++){
						...
						//$presvalue['SNO1']=$k+1;
						$agntvalue['PARTNER2']= '<a href="'.base_url().'reports/agent_turnover/agentpokerreport/'.$agent_results[$l]->PARTNER_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$partnername.'</a>';
...
jQuery("#listsg11").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/distpokerreport/<?php echo $partnerid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search',
	datatype: "xml",
	height: '100%',
	width: 1002,
...
jQuery("#listsg12").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/poker_subdistreport/<?php echo $partnerid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&GAMES_TYPE='+"<?php echo $sGameType;?>"+'&agentid='+"<?php echo $sAgentID;?>"+'&keyword=Search',
	datatype: "xml",
	height: '100%',
	width: 1002,
   	colNames:['Partner Name','Play Points', 'Win Points','Player Loss', 'Total Rake','Agent Rake','Company'],
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		$("#"+subgrid_id).html("<table id='"+subgrid_table_id+"' class='scroll'></table><div id='"+pager_id+"' class='scroll'></div>");
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/poker_subgrid_gamewise_subdistreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'',
			datatype: "xml",
			colNames: ['Game','Play Points','Win Points','Total Rake','Agent Rake','Margin%','Type'],
			colModel: [
				{name:"GAME_ID",index:"GAME_ID",width:180,key:true},
...
jQuery("#listsg13").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/poker_agent_report/<?php echo $distid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&GAMES_TYPE='+"<?php echo $sGameType;?>"+'&agentid='+"<?php echo $sAgentID;?>"+'&keyword=Search',
	datatype: "xml",
	height: '100%',
	width: 1002,
   	colNames:['Partner Name','Play Points', 'Win Points','Player Loss', 'Total Rake','Agent Rake','Company'],
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		$("#"+subgrid_id).html("<table id='"+subgrid_table_id+"' class='scroll'></table><div id='"+pager_id+"' class='scroll'></div>");
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/poker_subgrid_gamewise_agentreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'&distid=<?php echo $distid;?>',
			datatype: "xml",
			colNames: ['Game','Play Points','Win Points','Total Rake','Agent Rake','Margin%','Type'],
			colModel: [
				{name:"GAME_ID",index:"GAME_ID",width:180,key:true},
				{name:"PLAY_POINTS",index:"PLAY_POINTS",width:110,align:"right"},
				{name:"WIN_POINTS",index:"WIN_POINTS",width:130,align:"right"},
        {name:"TOTAL_RAKE",index:"TOTAL_RAKEMARGIN",width:130,align:"right"},
				{name:"MARGIN",index:"MARGIN",width:130,align:"right"},
				//{name:"NET",index:"NET",width:130,align:"right"},
				{name:"MARGIN_PERCENTAGE",index:"MARGIN_PERCENTAGE",width:130,align:"center"},
				{name:"TYPE",index:"TYPE",width:70,align:"left"}
	...
	loadComplete: function () {
		var playpointsSum = jQuery("#listsg13").jqGrid('getCol', 'PLAYPOINTS', false, 'sum').toFixed(2);
		var winpointsSum = jQuery("#listsg13").jqGrid('getCol', 'WINPOINTS', false, 'sum').toFixed(2);
		var playerloss   = jQuery("#listsg13").jqGrid('getCol', 'PLAYER_LOSS', false, 'sum').toFixed(2);
    	var totrake = jQuery("#listsg13").jqGrid('getCol', 'TOTAL_RAKE', false, 'sum').toFixed(2);
		var margin = jQuery("#listsg13").jqGrid('getCol', 'MARGIN', false, 'sum').toFixed(2);
		var settlement = jQuery("#listsg13").jqGrid('getCol', 'SETTLEMENT_AMOUNT', false, 'sum').toFixed(2);
		
		jQuery("#listsg13").jqGrid('footerData','set', {PARTNER: 'Total:', PLAYPOINTS: playpointsSum,WINPOINTS:winpointsSum,PLAYER_LOSS:playerloss,TOTAL_RAKE:totrake,MARGIN:margin, SETTLEMENT_AMOUNT:settlement});
	}
});
