...
class Agent_user_inpoints extends CI_Controller{
    ...
    function __construct(){
	  parent::__construct();
			...
			$this->load->model('reports/agentuserinpoints_model');
	...
	public function agent_user_inpointshistory(){
	  
		//general configuration
	
		$config['base_url']  = base_url()."reports/agent_user_inpoints/agent_user_inpointshistory/";
		...
		if($this->input->get_post('keyword',TRUE)=="Search"){		
			...
			$data['results']		= $this->agentuserinpoints_model->getAllAgentUserSearchLedger($loggedInPartnersList,$searchdata,$config["per_page"],$start);
...
//		echo "insdjkds";
		...
		$partners_list=$this->agentuserinpoints_model->getPartnersNameList($totPartnersList);
		$data['rid'] = $rid;
		$data['loggedInPartnersList'] = $partners_list;
		
		
		$this->load->view('reports/agent_userinpoints_details',$data);   
