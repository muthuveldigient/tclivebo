	...
	public function history(){
		...
		if($this->input->get_post('keyword',TRUE)=="Search"){		
				...
				$data['pagination']     = $this->pagination->create_links('view');
                                $this->session->set_userdata('gameHistorySearchData',$data['partner']);
		}elseif($start != ''){
			...
			if($data['username'] != ''){
				...
				if($user_id != ''){
				  ...
				  $data['user_id'] = '';
                        ...
                        $totCount = $this->report_model->getAllSearchGameCount($loggedInUsersPartnersId,$data);
		        $totPlayPoint = $this->report_model->getAllSearchGamePointsCount($loggedInUsersPartnersId,$data,"BET_POINTS");
			$totWinPoint = $this->report_model->getAllSearchGamePointsCount($loggedInUsersPartnersId,$data,"WIN_POINTS");
			$totRefundPoint = $this->report_model->getAllSearchGamePointsCount($loggedInUsersPartnersId,$data,"REFUND_POINTS");
                        
			$data['results'] = $this->report_model->getAllSearchGameInfo($loggedInUsersPartnersId,$data,$config["per_page"],0); 
			$data['tot_users']      = $totCount;
			$data['totPlayPoint']   = $totPlayPoint;
			$data['totWinPoint']    = $totWinPoint;
			$data['totRefundPoint'] = $totRefundPoint;                        
                 }else{
				$this->session->unset_userdata('gameHistorySearchData');
		}
		...
		//$this->load->view("user/search_users",$searchData);
            ...
            public function user(){
                ...
                $sdate = $_REQUEST['START_DATE_TIME'];
                $edate = $_REQUEST['END_DATE_TIME'];
	//echo $edate;die;
		//get all the partner ids
		$loggedInUsersPartnersId = $this->Agent_model->getAllChildIds($this->session->userdata); 
		...
		if($this->input->get_post('keyword',TRUE)=="Search"){		
			...
			$totCount       = $this->report_model->getAllUserGameCount($user_id,$sdate,$edate);
			$totPlayPoint   = $this->report_model->getAllUserGamePlaySum($user_id,$sdate,$edate,"BET_POINTS");
			$totWinPoint    = $this->report_model->getAllUserGamePlaySum($user_id,$sdate,$edate,"WIN_POINTS");
			$totRefundPoint = $this->report_model->getAllUserGamePlaySum($user_id,$sdate,$edate,"REFUND_POINTS");
			...
			$data['START_DATE_TIME']= $this->input->get_post('START_DATE_TIME',TRUE);
			$data['END_DATE_TIME']	= $this->input->get_post('END_DATE_TIME',TRUE);
			$data['results']	= $this->report_model->getAllUserGameInfo($user_id,$sdate,$edate);
			$data['tot_users']      = $totCount;
			$data['totPlayPoint']   = $totPlayPoint;
			$data['totWinPoint']    = $totWinPoint;
			$data['totRefundPoint'] = $totRefundPoint;
		...
		$this->load->view('reports/gamepop',$data);                
            ...
            public function oddgroup(){
                $data['results'] = $this->report_model->getParticularGroupIdGameInfo($_REQUEST[groupid]);
                $data['game'] = ($_REQUEST[gameid]);
                $this->load->view('reports/groupidpop',$data); 
            }
            
            public function luckgroup(){
                $data['results'] = $this->report_model->getParticularLuckGroupGameInfo($_REQUEST[groupid]);
                $data['game'] = ($_REQUEST[gameid]);
                $this->load->view('reports/groupidpop',$data);                 
            }
            
            public function luckynumgroup(){
                $data['results'] = $this->report_model->getParticularluckynumGroupGameInfo($_REQUEST[groupid]);
                $data['game'] = ($_REQUEST[gameid]);
                $this->load->view('reports/groupidpop',$data);                                 
            }
