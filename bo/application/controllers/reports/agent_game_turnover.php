...
class Agent_game_turnover extends CI_Controller{
	 ...
	 public function subdistreport_details(){
		...
		if($loggedinPartnerIDs)
		 ...
		 $subdistid=$this->uri->segment(4,0);
		...
		if($this->input->get_post('keyword',TRUE)=="Search"){
			...
			$data['gameid'] = $this->uri->segment(5,0);
			$data["partnertype"] = $this->session->userdata('partnertypeid');
			$data["dresults"] = $this->agentreport_model->getSubDistributorAllGameTurnover($data);
		}
		
		$this->load->view('reports/agent_subdist_game_turnover',$data);  
	 }
	 ...
	 public function agentreport(){
		...
		if($this->input->get_post('keyword',TRUE)=="Search"){
			...
			$data['gameid'] = $this->uri->segment(5,0);
			$data["partnertype"] = $this->session->userdata('partnertypeid');
			$data["agntresults"] = $this->agentreport_model->getAgentGameTurnover($data);
		}
		
		$this->load->view('reports/agent_game_user_turnover',$data);  
	 }	
