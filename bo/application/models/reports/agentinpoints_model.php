...
/*
  ...
  Author 	    : Sivakumar
*/
class agentinpoints_Model extends CI_Model
{
	...
	public function getInpointsInfo($loggedInPartnersList,$searchdata,$limitend,$limitstart) {
		$userid="";
		...
		$this->db2->where_in('l.TRANSACTION_STATUS_ID',array(103,130));
	...
	public function getAllSearchLedgerCount($loggedInPartnersList,$searchdata) {
		...
		if($sql){
			if($loggedInPartnersList){
				if(is_array($loggedInPartnersList)){
					$loggedInPartnersList1=implode(",",$loggedInPartnersList);
		$querycnt=$this->db2->query("select count(*) as cnt from partners_transaction_details pt  where $sql and pt.PARTNER_ID in (".$loggedInPartnersList1.") and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID ");
				}else{
		$querycnt=$this->db2->query("select count(*) as cnt from partners_transaction_details pt  where $sql and pt.PARTNER_ID in (".$loggedInPartnersList.") and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID ");		
				}
			}else{
		$querycnt=$this->db2->query("select count(*) as cnt from partners_transaction_details pt  where $sql and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");	
			}
		}else{
			if($loggedInPartnersList){
				if(is_array($loggedInPartnersList)){
					$loggedInPartnersList1=implode(",",$loggedInPartnersList);
		$querycnt=$this->db2->query("select count(*) as cnt from partners_transaction_details pt and pt.PARTNER_ID in (".$loggedInPartnersList1.") and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");
				}else{
$querycnt=$this->db2->query("select count(*) as cnt from partners_transaction_details pt and pt.PARTNER_ID in (".$loggedInPartnersList.") and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");				
				}
			}else{
		$querycnt=$this->db2->query("select count(*) as cnt from partners_transaction_details pt and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");	
			}
		}
	...
	public function getAllSearchLedger($loggedInPartnersList,$searchdata,$limitend,$limitstart) {
		...
		if($searchdata['username']){
          ...
          if(($searchdata['agent_list']!='all' && $searchdata['agent_list']!='')  || $searchdata['sdate']!=''  || $searchdata['edate']!=''){
              ...
              if($partid!=''){
                  if($searchdata['processed_by']){  
           				$conQuery=$conQuery."  (pt.PROCESSED_BY='".$searchdata['processed_by']."')";                             
                  }else{
        			  $conQuery=$conQuery." (pt.PARTNER_ID='".$partid."' AND SUB_PARTNER_ID=0)";                                
                  }  
              }
		...
		if($sql){
			if($loggedInPartnersList){
				if(is_array($loggedInPartnersList)){
					$loggedInPartnersList1=implode(",",$loggedInPartnersList);
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,sum(pt.AMOUNT) as AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList1.") and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");
				}else{
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,sum(pt.AMOUNT) as AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList.") and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");
				}
			}else{
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,sum(pt.AMOUNT) as AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY from partners_transaction_details pt   where $sql  and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");	
			}
		}else{
			if($loggedInPartnersList){
				if(is_array($loggedInPartnersList)){
		...
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,sum(pt.AMOUNT) as AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY from partners_transaction_details pt and TRANSACTION_STATUS_ID in(103,107,130)  group by pt.PROCESSED_BY,pt.PARTNER_ID,pt.USER_ID,pt.SUB_PARTNER_ID");	
			}
		}
	...
	public function getAllAgentSearchLedger($loggedInPartnersList,$searchdata,$limitend,$limitstart) {
		...
		$conQuery="";
		if($searchdata['agent_list']!='all' && $searchdata['agent_list']!=''){
			if($session_data['partnertypeid']==11 || $session_data['partnertypeid']==12 || $session_data['partnertypeid']==13){
				$partnerids=$this->getAllChildPartner($searchdata['agent_list']);
				if(isset($partnerids)){
					$conQuery=$conQuery." pt.PARTNER_ID in(".$partnerids.")";  
				}
			...
			if($loggedInPartnersList){
			   if(is_array($loggedInPartnersList)){
					...
					if($searchdata['processed_by']){
			  		...
			  		if($searchdata['processed_by']){
						$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=0 and pt.PROCESSED_BY='$processed_by' and pt.TRANSACTION_STATUS_ID in(103,107,130) ORDER BY pt.PARTNER_TRANSACTION_ID DESC");
					}else{
						if($partid){
						$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=0 and pt.TRANSACTION_STATUS_ID in(103,107,130) ORDER BY pt.PARTNER_TRANSACTION_ID DESC");
						}else{
						  if($partnerid){	
						$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=0 and pt.TRANSACTION_STATUS_ID in(103,107,130) and pt.PARTNER_ID='".$partnerid."' ORDER BY pt.PARTNER_TRANSACTION_ID DESC");
						  }else{
						$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=0 and pt.TRANSACTION_STATUS_ID in(103,107,130) ORDER BY pt.PARTNER_TRANSACTION_ID DESC");  
						  }
						}
		...
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and TRANSACTION_STATUS_ID in(103,107,130) and pt.USER_ID=0 ORDER BY pt.PARTNER_TRANSACTION_ID DESC");	
			}
		}else{
			if($loggedInPartnersList){
			  if(is_array($loggedInPartnersList)){
		...
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt  where pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=0 and pt.TRANSACTION_STATUS_ID in(103,107,130) and pt.PARTNER_ID='".$partnerid."' ORDER BY pt.PARTNER_TRANSACTION_ID DESC");		
				...
				if($partid){
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt and TRANSACTION_STATUS_ID in(103,107,130) and pt.USER_ID=0 ORDER BY pt.PARTNER_TRANSACTION_ID DESC");	
				}else{
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt and TRANSACTION_STATUS_ID in(103,107,130) and pt.USER_ID=0 and pt.PARTNER_ID='".$partnerid."' ORDER BY pt.PARTNER_TRANSACTION_ID DESC");			
				}
			}
		}
