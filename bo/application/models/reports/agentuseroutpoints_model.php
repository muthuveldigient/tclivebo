...
/*
  ...
  Author 	    : Sivakumar
*/
class agentuseroutpoints_Model extends CI_Model
{
			   ...
			   if(is_array($loggedInPartnersList)){
					...
					if($searchdata['processed_by']){
			  		...
			  		if($searchdata['processed_by']){
						...
						$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT ,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=$uid and pt.TRANSACTION_STATUS_ID in(131) ORDER BY pt.PARTNER_TRANSACTION_ID DESC");						  						 }

					}
			  }

			}else{
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt   where $sql  and TRANSACTION_STATUS_ID in(131) and pt.USER_ID=$uid and pt.PARTNER_ID=$partnerid ORDER BY pt.PARTNER_TRANSACTION_ID DESC");
			}
		}else{
			if($loggedInPartnersList){
			  if(is_array($loggedInPartnersList)){
					$loggedInPartnersList1=implode(",",$loggedInPartnersList);
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt  where pt.PARTNER_ID in (".$loggedInPartnersList1.") and pt.USER_ID=$uid and pt.TRANSACTION_STATUS_ID in(131) and pt.PARTNER_ID='".$partnerid."' ORDER BY pt.PARTNER_TRANSACTION_ID DESC");

			  }else{
		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt  where pt.PARTNER_ID in (".$loggedInPartnersList.") and pt.USER_ID=$uid and pt.TRANSACTION_STATUS_ID in(131) and pt.PARTNER_ID='".$partnerid."' ORDER BY pt.PARTNER_TRANSACTION_ID DESC");

			  }
			}else{

		$query=$this->db2->query("select pt.PARTNER_ID,pt.TRANSACTION_TYPE_ID,pt.TRANSACTION_STATUS_ID,pt.USER_ID,pt.AMOUNT,pt.INTERNAL_REFERENCE_NO,pt.CURRENT_TOT_BALANCE,pt.CLOSING_TOT_BALANCE,pt.CREATED_TIMESTAMP,pt.PROCESSED_BY,pt.SUB_PARTNER_ID from partners_transaction_details pt and TRANSACTION_STATUS_ID in(131) and pt.USER_ID=$uid and pt.PARTNER_ID=$partnerid ORDER BY pt.PARTNER_TRANSACTION_ID DESC");	
			}
		}
