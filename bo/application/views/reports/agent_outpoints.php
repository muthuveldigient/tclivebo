...
<div class="content_wrap">
	...
	echo form_open('reports/agent_outpoints/outpointshistory?rid='.$rid,$attributes);
...
if($agent_list!='all' && $agent_list!=''){
	$getselectedagent=$this->agentoutpoints_model->getPartnerId($agent_list);
}
	...
	if(count($results)>0 && is_array($results)){
		...
		for($i=0;$i<count($results);$i++){
			  ...
			  if($results[$i]->USER_ID!="0"){  
					$sqlusername=mysql_query("select USERNAME,PARTNER_ID from user where USER_ID='".$USER_ID."'");
					$rowusername=mysql_fetch_array($sqlusername);
					$userdetails=$this->agentoutpoints_model->getUserById($results[$i]->USER_ID);
					...
					if($results[$i]->INTERNAL_REFERENCE_NO){
						...
						$userbalance=$this->agentoutpoints_model->getUserBalance($sdate,$edate,$results[$i]->USER_ID,$results[$i]->PROCESSED_BY);
					   ...
					   // echo "Current total".$row_userbalance['CURRENT_TOT_BALANCE'];
						...
						$inpoints_det=$this->agentoutpoints_model->getOutpoints($sdate,$edate,$results[$i]->USER_ID,$results[$i]->PARTNER_ID,$results[$i]->PROCESSED_BY);
						
						$AMOUNT=$inpoints_det[0]->AMOUNT;
						$resultValue['AMOUNT_OUT'] = number_format($AMOUNT,2,".",""); 
						$resultValue1['AMOUNT_OUT'] = $AMOUNT;
			  ...
			  if($results[$i]->SUB_PARTNER_ID!="0" && $results[$i]->SUB_PARTNER_ID!=''){
				...
				$partnername=$this->agentoutpoints_model->getPartnerId($results[$i]->SUB_PARTNER_ID);
				...
				$resultValue['USERNAME'] = $partnername[0]->PARTNER_USERNAME;
				  ...
				  $partnerdetails=$this->agentoutpoints_model->getPartnerId($results[$i]->PARTNER_ID);
				  ...
				  if($session_data['partnertypeid']=="11"){
					...
					$partnerusername=$this->agentoutpoints_model->getPartnerId($searpartnerid);
					...
					$partnertransbal=$this->agentoutpoints_model->getPartnertransBal($results[$i]->SUB_PARTNER_ID,$results[$i]->INTERNAL_REFERENCE_NO);
					...
					$inpointsdet=$this->agentoutpoints_model->getInpointsLess($sdate,$edate,$partnerusername[0]->PARTNER_ID,$results[$i]->PROCESSED_BY);
				  ...
				  if($session_data['partnertypeid']=="11"){
					...
					$rowpartner=mysql_fetch_array($sqlpartner);
					$partnerusername=$this->agentoutpoints_model->getPartnerId($searpartnerid);
					$resultValue['USERNAME'] = "<a href='".base_url()."reports/agent_outpoints/agent_outpointshistory?rid=".$rid."&parid=".base64_encode($results[$i]->PARTNER_ID)."&START_DATE_TIME=".date("d-m-Y H:i:s",strtotime($sdate))."&END_DATE_TIME=".date("d-m-Y H:i:s",strtotime($edate))."&keyword=Search'>".$partnerusername[0]->PARTNER_USERNAME."</a>";
					$resultValue1['USERNAME'] = $partnerusername[0]->PARTNER_USERNAME;
			...
			if($resultValue1['USERNAME']!="" && $resultValue1['PARTNER_USERNAME']!=""){
				...
				$partnerdet=$this->agentoutpoints_model->getPartnerIdByUsername($resultValue1['USERNAME']);
				if($partnerdet[0]->PARTNER_ID){
				
					
					
					$cumulativeamt=$this->agentoutpoints_model->getCumulativeAmount($sdate,$edate,$partnerdet[0]->PARTNER_ID,$resultValue1['PARTNER_USERNAME']);
					
					$resultValue['AMOUNT_OUT']=number_format($cumulativeamt[0]->AMOUNT1,2,".","");
					$resultValue1['AMOUNT_OUT']=$cumulativeamt[0]->AMOUNT1;
				}
			}
			
			
			
			if($resultValue['AMOUNT_OUT']!="" && $resultValue['AMOUNT_OUT']!="0.00" && !in_array($resultValue1['USERNAME']."-".$resultValue1['PARTNER_USERNAME'],$getValues) && $resultValue1['USERNAME']!=$resultValue1['PARTNER_USERNAME'] ){   
...
if($arr){
    for($j=0;$j<count($arr);$j++){
        $totalpoints=$totalpoints+$arr[$j]['AMOUNT_OUT'];
    }
}
