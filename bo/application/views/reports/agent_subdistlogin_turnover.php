...
<div class="MainArea"> <?php echo $this->load->view("common/sidebar"); ?>
  <div class="RightWrap">
		...
		$agent_results 	 = $results['agent_results'];
			...
			if(count($agent_results)>0 && is_array($agent_results)){
                      for($l=0;$l<count($agent_results);$l++){
							//get partner name
							$partnername	  = '<a href="'.base_url().'reports/agent_turnover/agentreport/'.$agent_results[$l]->PARTNER_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&GAMES_TYPE='.$GAMES_TYPE.'&sdate='.$START_DATE_TIME.'&AGENT_LIST='.$AGENT_LIST.'&keyword=Search">'.$agent_results[$l]->PARTNER_NAME.'</a>';
						//get partner revenueshare
						//$share	 = $this->Agent_model->getRevenueShareByPartnerId($results[$i]->partner_id);
				 ...
				 if(isset($PARTNER_TYPE) && !empty($PARTNER_TYPE)){ //only set post PARTNER_TYPE 
					 $arrsp =1;
				 }else{
                     $arrsp="";
				 }
	...
	font-size:13px;
        ...
        <div class="tableListWrap">
          <div class="PageHdr"><b><?php echo (!empty($PARTNER_TYPE)?'Sub Distributor':'Self')?></b></div>
          <div class="data-list">
...
jQuery("#listsg12").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/subdistreport/<?php echo $partnerid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&GAMES_TYPE='+"<?php echo $GAMES_TYPE; ?>"+'&AGENT_LIST='+"<?php echo $AGENT_LIST; ?>"+'&PARTNER_TYPE='+"<?php echo $PARTNER_TYPE; ?>"+'&keyword=Search', 
	datatype: "xml",
	height: '100%',
	width: 1002,
...
jQuery("#listsg13").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/agent_report/<?php echo $distid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&GAMES_TYPE='+"<?php echo $GAMES_TYPE; ?>"+'&AGENT_LIST='+"<?php echo $AGENT_LIST; ?>"+'&PARTNER_TYPE='+"<?php echo $PARTNER_TYPE; ?>"+'&keyword=Search', 
	datatype: "xml",
	height: '100%',
	width: 1002,
   	colNames:['Partner Name','Total Bets', 'Total Wins', 'Agent','Company'],
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		$("#"+subgrid_id).html("<table id='"+subgrid_table_id+"' class='scroll'></table><div id='"+pager_id+"' class='scroll'></div>");
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/subgrid_gamewise_agentreport?q=2&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'&distid=<?php echo $distid;?>',
			datatype: "xml",
			colNames: ['Game','Play Points','Win Points','Agent','Company','Margin%','Type'],
			colModel: [
				{name:"GAME_ID",index:"GAME_ID",width:160,key:true},
