...
<div class="content_wrap">
	...
	echo form_open('reports/agent_ledger/ledgerhistory?rid='.$rid,$attributes);
...
if(isset($results)){ 
	if(count($results)>0 && is_array($results)){
		...
		//call model to get Tournament Name,currency type.
			...
			if($results[$i]->USER_ID){
				...
				if($results[$i]->INTERNAL_REFERENCE_NO){
					$userbalance_details=$this->agentledger_model->getUserClosingBal($results[$i]->INTERNAL_REFERENCE_NO);
					$CURRENT_TOT_BALANCE=$userbalance_details[0]->CURRENT_TOT_BALANCE;
					$CLOSING_TOT_BALANCE=$userbalance_details[0]->CLOSING_TOT_BALANCE;
				}    
			}else{
				 
				  $partnerdetails=$this->agentledger_model->getPartnerId($results[$i]->PARTNER_ID);
				  ...
				  $partnerusername=$this->agentledger_model->getPartnerId($searpartnerid);
				  $resultValue['USERNAME'] = $partnerusername[0]->PARTNER_USERNAME;
			}
			
			
													
			if($CURRENT_TOT_BALANCE<$CLOSING_TOT_BALANCE){ $resultValue['AMOUNT_IN'] = number_format($results[$i]->AMOUNT,2,".",""); } else if($CURRENT_TOT_BALANCE==$CLOSING_TOT_BALANCE){ $resultValue['AMOUNT_IN'] = '0';} else $resultValue['AMOUNT_IN'] = '';
			
			if($CURRENT_TOT_BALANCE>$CLOSING_TOT_BALANCE){ $resultValue['AMOUNT_OUT'] = number_format($results[$i]->AMOUNT,2,".",""); } else $resultValue['AMOUNT_OUT'] = '';
			...
			if($results[$i]->USER_ID=='0'){
					...
					$adjustment_details=$this->agentledger_model->getAdjustmentDetById($results[$i]->INTERNAL_REFERENCE_NO,$results[$i]->PARTNER_ID);
					
					if($adjustment_details[0]->ADJUSTMENT_CREATED_BY){
							if($results[$i]->PARTNER_ID!=$session_data['partnerid']){
							 if($resultValue['USERNAME']==$adjustment_details[0]->ADJUSTMENT_CREATED_BY){
								$adjustt=$this->agentledger_model->getAdjustmentDetByRefId($results[$i]->INTERNAL_REFERENCE_NO);
						...
						if($results[$i]->USER_ID=='0' && $results[$i]->SUB_PARTNER_ID=='0'){
								$partnerdetails=$this->agentledger_model->getPartnerId($results[$i]->PARTNER_ID);
								if($partnerdetails[0]->FK_PARTNER_TYPE_ID=="1"){
									$resultValue['PARTNER_USERNAME'] = "Admin";
								}else{
									$resultValue['PARTNER_USERNAME'] = $results[$i]->PROCESSED_BY;    
								}
							...
							if($results[$i]->PARTNER_ID!=$session_data['partnerid']){
								$partnerdetails=$this->agentledger_model->getPartnerId($results[$i]->PARTNER_ID);
								$resultValue['PARTNER_USERNAME'] = $partnerdetails[0]->PARTNER_NAME;
							}else{
								if($results[$i]->PROCESSED_BY){
								$resultValue['PARTNER_USERNAME']=$results[$i]->PROCESSED_BY;    
								}else{
		...
		if(substr($adjustment_details[0]->ADJUSTMENT_COMMENT,0,20)) { $resultValue['ADJUSTMENT_COMMENT'] = substr($adjustment_details[0]->ADJUSTMENT_COMMENT,0,20); }else{ $resultValue['ADJUSTMENT_COMMENT'] = ''; }	
			...
			if($resultValue['PARTNER_USERNAME']==""){
   				$resultValue['PARTNER_USERNAME']=$result[$i]->PROCESSED_BY; 
			}

			if($processed_by!="" && $username==''){
				if($processed_by==$resultValue['PARTNER_USERNAME']){
					$arr[] = $resultValue;    
				}
			}elseif($processed_by!="" && $username!=''){
				if( ($processed_by==$resultValue['PARTNER_USERNAME']) && ($username==$resultValue['USERNAME']) ){
					$arr[] = $resultValue;    
				}
			}else{
			 if($agent_list){   
			  if($agent_list==$result[$i]->PARTNER_ID){
				if($result[$i]->USER_ID!="0" || $result[$i]->SUB_PARTNER_ID!="0"){  
				$arr[] = $resultValue;    
				}
			  }else{
				$arr[] = $resultValue;      
