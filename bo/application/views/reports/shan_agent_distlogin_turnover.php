...
<?php	$self_results 	 = $results['self_results'];
		...
		if(count($subdist_results)>0 && is_array($subdist_results)){
			for($l=0;$l<count($subdist_results);$l++){
				...
				$totalrake  = $subdist_results[$i]->TOTAL_RAKE;
				$playerloss = $subdist_results[$i]->PLAYER_LOSS;
				$settleamount= $subdist_results[$i]->SETTLEMENT_AMOUNT;				
				
				$commission = $subdist_results[$l]->MARGIN;
				$partner_comm = $subdist_results[$l]->NET;						
				//$presvalue['SNO1']=$k+1;
				$presvalue['PARTNER1']= '<a href="'.base_url().'reports/agent_turnover/agentreport/'.$subdist_results[$l]->PARTNER_ID.'?rid=19&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'">'.$partnername.'</a>';
		...
		if(count($agent_results)>0 && is_array($agent_results)){
			for($l=0;$l<count($agent_results);$l++){
				...
				//$presvalue['SNO1']=$k+1;
				$agntvalue['PARTNER2']= '<a href="'.base_url().'reports/agent_turnover/agentreport/'.$agent_results[$l]->PARTNER_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$partnername.'</a>';
...
jQuery("#listsg11").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/distshanreport/<?php echo $partnerid;?>?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search', 
	datatype: "xml",
	height: '100%',
	width: 1002,
