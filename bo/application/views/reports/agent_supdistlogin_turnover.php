    ...
    $resvalues=array();
        	...
        	if(count($results)>0 && is_array($results)){
				for($i=0;$i<count($results);$i++){
					...
					//$resvalue['SNO']=$j+1;
					if($partnertype==15){
						$resvalue['PARTNER']= '<a href="'.base_url().'reports/agent_turnover/distreport/'.$results[$i]->DISTRIBUTOR_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&GAMES_TYPE='.$GAMES_TYPE.'&PARTNER_TYPE='.$PARTNER_TYPE.'&edate='.$END_DATE_TIME.'&AGENT_LIST='.$AGENT_LIST.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$partnername.'</a>';
					}else{
						$resvalue['PARTNER']= '<a href="'.base_url().'reports/agent_turnover/distreport/'.$results[$i]->MAIN_AGEN_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&GAMES_TYPE='.$PARTNER_TYPE.'&PARTNER_TYPE='.$PARTNER_TYPE.'&edate='.$END_DATE_TIME.'&AGENT_LIST='.$AGENT_LIST.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$partnername.'</a>';
					}
...
.searchWrap1 { background-color: #F8F8F8;    border: 1px solid #EEEEEE;    border-radius: 5px;    float: left;    width: 100%;	margin-top:10px;	font-size:13px;  }
			...
			<div class="tableListWrap">
          	...
          	<div class="data-list">
				...
				url:'<?php echo base_url();?>reports/ajax/subgrid_report?q=1&GAMES_TYPE='+"<?php echo $GAMES_TYPE; ?>"+'&PARTNER_TYPE='+"<?php echo $PARTNER_TYPE; ?>"+'&AGENT_LIST='+"<?php echo $AGENT_LIST; ?>"+'&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search', 
				datatype: "xml",
				height: '100%',
				width: 1002,
				...
				var aurls = "<?php echo base_url();?>reports/ajax/subgrid_gamewise_report?q=2&GAMES_TYPE="+'<?php echo $GAMES_TYPE; ?>'+"&PARTNER_TYPE="+'<?php echo $PARTNER_TYPE; ?>'+"&AGENT_LIST='<?php echo $AGENT_LIST; ?>'&gtype=0&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+''; 
				<?php if($_REQUEST['PARTNER_TYPE'] == 13){ ?>
					var aurls = "<?php echo base_url();?>reports/ajax/subgrid_gamewise_subdistreport?q=2&GAMES_TYPE="+'<?php echo $GAMES_TYPE; ?>'+"&PARTNER_TYPE="+'<?php echo $PARTNER_TYPE; ?>'+"&AGENT_LIST='<?php echo $AGENT_LIST; ?>'&gtype=0&pname="+partner_name+"&id="+row_id+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+''; 
				<?php }?>
				...
				var playpointsSum = jQuery("#listsg11").jqGrid('getCol', 'PLAY_POINTS', false, 'sum');
				var winpointsSum = jQuery("#listsg11").jqGrid('getCol', 'WIN_POINTS', false, 'sum');
...
jQuery("#listsg11").jqGrid({
	...
	height: '100%',
	width: 1002,
   	colNames:['Partner Name','Play Point', 'Win Point', 'Margin','Net'],
   	colModel:[
   		{name:'PARTNER',index:'PARTNER', width:135,sorttype: "text"},
   		{name:'PLAY_POINTS',index:'PLAY_POINTS', width:115,sorttype: "float",align:"right",summaryType: 'sum',  formatter: 'float'},
   		{name:'WIN_POINTS',index:'WIN_POINTS', width:115,sorttype: "float",align:"right",summaryType: 'sum',  formatter: 'float'},
