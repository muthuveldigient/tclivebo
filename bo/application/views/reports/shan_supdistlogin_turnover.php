...
<?php	
//echo "<pre>"; print_r($results['self_results']); die;
    ...
    $resvalues=array();
        	...
        	if(count($results)>0 && is_array($results)){
				for($i=0;$i<count($results);$i++){
					...
					$totalrake  = $results[$i]->TOTAL_RAKE;
					$playerloss = $results[$i]->PLAYER_LOSS;
					$settleamount= $results[$i]->SETTLEMENT_AMOUNT;					
					...
					//$resvalue['SNO']=$j+1;
					if($partnertype==15){
						$resvalue['PARTNER']= '<a href="'.base_url().'reports/agent_turnover/distreport/'.$results[$i]->DISTRIBUTOR_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$partnername.'</a>';
					}else{
						$resvalue['PARTNER']= '<a href="'.base_url().'reports/agent_turnover/distreport/'.$results[$i]->MAIN_AGEN_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&keyword=Search">'.$partnername.'</a>';
					}
...
.searchWrap1 { background-color: #F8F8F8;    border: 1px solid #EEEEEE;    border-radius: 5px;    float: left;    width: 100%;	margin-top:10px;	font-size:13px;  }
			...
			<div class="tableListWrap">
          	...
          	<div class="data-list">
				...
				url:'<?php echo base_url();?>reports/ajax/shan_subgrid_report?q=1&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search', 
				datatype: "xml",
				height: '100%',
				width: 1002,
				...
				var totalrakeSum = jQuery("#listsg11").jqGrid('getCol', 'TOTAL_RAKE', false, 'sum').toFixed(2);
				var totalploss = jQuery("#listsg11").jqGrid('getCol', 'PLAYER_LOSS', false, 'sum').toFixed(2);
				var totalsettlement = jQuery("#listsg11").jqGrid('getCol', 'SETTLEMENT_AMOUNT', false, 'sum').toFixed(2);				
				...
				jQuery("#listsg11").jqGrid('footerData','set', {PARTNER: 'Total:', PLAY_POINTS: playpointsSum,WIN_POINTS:winpointsSum, PLAYER_LOSS:totalploss,TOTAL_RAKE:totalrakeSum, MARGIN:margin, SETTLEMENT_AMOUNT:totalsettlement    });	
				}
				});
...
jQuery("#listsg11").jqGrid({
	...
	height: '100%',
	width: 1002,
   	colNames:['Partner Name','Play Points', 'Win Points','Player Loss', 'Total Rake','Agent Rake','Settlement'],
   	colModel:[
   		{name:'PARTNER',index:'PARTNER', width:135,sorttype: "text"},
   		{name:'PLAY_POINTS',index:'PLAY_POINTS', width:115,sorttype: "float",align:"right",summaryType: 'sum',  formatter: 'float'},
   		{name:'WIN_POINTS',index:'WIN_POINTS', width:115,sorttype: "float",align:"right",summaryType: 'sum',  formatter: 'float'},
