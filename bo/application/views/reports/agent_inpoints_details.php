...
if(isset($results)){ 
	if(count($results)>0 && is_array($results)){
		...
		for($i=0;$i<count($results);$i++){
		   if($results[$i]->CURRENT_TOT_BALANCE<$results[$i]->CLOSING_TOT_BALANCE){ 
			  ...
			  if($results[$i]->USER_ID!="0"){  
					...
					if($results[$i]->INTERNAL_REFERENCE_NO){
						$userbalance=$this->agentinpoints_model->getUserBalanceById($results[$i]->INTERNAL_REFERENCE_NO);
			  ...
			  if($results[$i]->SUB_PARTNER_ID!="0" && $results[$i]->SUB_PARTNER_ID!=''){
				...
				$partnername=$this->agentinpoints_model->getPartnerId($results[$i]->PARTNER_ID);
				...
				}elseif($results[$i]->CURRENT_TOT_BALANCE<$results[$i]->CLOSING_TOT_BALANCE){
			   ...
			   if($session_data['partnertypeid']=="11"){   
						if($processed_by){
							if($processed_by==$resultValue1['PARTNER_USERNAME']){
								if($resultValue1['USERNAME']!=$session_data['partnerusername']){
									if($totalpoints){
									$totalpoints=$totalpoints+$resultValue['AMOUNT_IN'];
									}else{
									$totalpoints=$resultValue['AMOUNT_IN'];    
									}
								}   
							}
						}else{
    ...
    <div class="tableListWrap">
      <div class="data-list">
        ...
        <script type="text/javascript">
                jQuery("#list4").jqGrid({
                    datatype: "local",
						colNames:['Trans.Id','Date', 'User Id','Old Points','In','New Points','Processed By', 'Comment'],
   						colModel:[
