...
if($agent_list!='all' && $agent_list!=''){
	$getselectedagent=$this->agentuseroutpoints_model->getPartnerId($agent_list);
}
	...
	if(count($results)>0 && is_array($results)){
		...
		for($i=0;$i<count($results);$i++){
			  ...
			  if($results[$i]->USER_ID!="0"){  
					...
					$userdetails=$this->agentuseroutpoints_model->getUserById($results[$i]->USER_ID);
				    
				
					if($results[$i]->INTERNAL_REFERENCE_NO){
						$userbalance=$this->agentuseroutpoints_model->getUserBalanceById($results[$i]->INTERNAL_REFERENCE_NO);
			...
			if($resultValue['AMOUNT_OUT']!="" && $resultValue['AMOUNT_OUT']!="0.00"){   
