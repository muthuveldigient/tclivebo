	...
	//echo '<pre>'; print_r($self_results); die;
		...
		if(count($user_results)>0 && is_array($user_results)){
			for($l=0;$l<count($user_results);$l++){
				...
				//$presvalue['SNO1']=$k+1;
				$presvalue['USER_NAME']= '<a href="'.base_url().'games/history/game/userhistory/?rid='.$rid.'&START_DATE_TIME='.$START_DATE_TIME.'&GAMES_TYPE='.$GAMES_TYPE.'&PARTNER_TYPE='.$PARTNER_TYPE.'&AGENT_LIST='.$AGENT_LIST.'&END_DATE_TIME='.$END_DATE_TIME.'&keyword=Search&playerID='.$partnername.'">'.$partnername.'</a>';
				//'<a href="'.base_url().'reports/agent_game_history/usergame_history/?rid='.$rid.'&START_DATE_TIME='.$START_DATE_TIME.'&END_DATE_TIME='.$END_DATE_TIME.'&keyword=Search&playerID='.$partnername.'">'.$partnername.'</a>';
...
jQuery("#listsg11").jqGrid({
   	url:'<?php echo base_url();?>reports/ajax/subgrid_report?q=1&gtype=0&PARTNER_TYPE='+"<?php echo $PARTNER_TYPE; ?>"+'&AGENT_LIST='+"<?php echo $AGENT_LIST; ?>"+'&GAMES_TYPE='+"<?php echo $GAMES_TYPE; ?>"+'&START_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>"+'&END_DATE_TIME='+"<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>"+'&keyword=Search', 
	datatype: "xml",
	height: '100%',
	width: 1002,
	...
	subGridRowExpanded: function(subgrid_id, row_id) {
		...
		jQuery("#"+subgrid_table_id).jqGrid({
			url:"<?php echo base_url();?>reports/ajax/subgrid_gamewise_agentreport?q=2&gtype=0&pname="+partner_name+"&id="+row_id+"&PARTNER_TYPE="+'<?php echo $PARTNER_TYPE; ?>'+"&AGENT_LIST="+'<?php echo $AGENT_LIST; ?>'+"&GAMES_TYPE="+'<?php echo $GAMES_TYPE; ?>'+"&START_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($START_DATE_TIME)); ?>'+"&END_DATE_TIME="+'<?php echo date("Y-m-d H:i:s",strtotime($END_DATE_TIME)); ?>'+'&keyword=Search'+'',
			datatype: "xml",
