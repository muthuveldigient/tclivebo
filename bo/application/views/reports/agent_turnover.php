    ...
    $resvalues=array();
        	...
        	if(count($results)>0 && is_array($results)){
				for($i=0;$i<count($results);$i++){
					...
					//$resvalue['SNO']=$j+1;
					if($partnertype==11){
						$resvalue['PARTNER']= '<a href="'.base_url().'reports/agent_turnover/distreport/'.$results[$i]->DISTRIBUTOR_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&AGENT_LIST='.$AGENT_LIST.'&GAMES_TYPE='.$GAMES_TYPE.'&PARTNER_TYPE='.$PARTNER_TYPE.'&keyword=Search">'.$partnername.'</a>';
					}else{
						$resvalue['PARTNER']= '<a href="'.base_url().'reports/agent_turnover/distreport/'.$results[$i]->MAIN_AGEN_ID.'?rid='.$rid.'&sdate='.$START_DATE_TIME.'&edate='.$END_DATE_TIME.'&AGENT_LIST='.$AGENT_LIST.'&GAMES_TYPE='.$GAMES_TYPE.'&PARTNER_TYPE='.$PARTNER_TYPE.'&keyword=Search">'.$partnername.'</a>';
					}
